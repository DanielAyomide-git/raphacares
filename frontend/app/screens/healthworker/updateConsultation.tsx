...
const AddConsultations: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Get the consultation_id from AsyncStorage
      const consultationId = await AsyncStorage.getItem('consultation_id');
      if (!consultationId) {
        Alert.alert('Error', 'Consultation ID not found in storage.');
        return;
      }
  
      // Get the appointment_id from AsyncStorage
      ...
      // Make the PUT request to update the consultation
      await axios.put(
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.label}>Follow-Up Start Date (YYYY-MM-DD):</Text>
      <TextInput
        style={styles.input}
      ...
      <Text style={styles.label}>Follow-Up End Date (YYYY-MM-DD):</Text>
      <TextInput
        style={styles.input}
...
const styles = StyleSheet.create({
  container: { padding: 16, backgroundColor: '#f7f9fc', marginTop:50 },
    label: { fontSize: 16, marginBottom: 8, fontWeight: 'bold',     marginRight: 30,    color: '#FFB815',
